...
module.exports = class Reverso {
    ...
    async getContext(
        ...
        if (cb && typeof cb !== 'function') {
            return {
                ok: false,
                message: 'getContext: cb pa rameter must be type of function',
            }
        }
	...
	let rUrl = this.CONTEXT_URL + [source, target].join('-') +'/' +encodeURIComponent(text).replace(/%20/g, '+')
        let sHtml = (await execute('curl_chrome116 ' + rUrl)) 


	const $ = load(sHtml.stdout) 
//	const $ = load('<h1 class=text>Hello, world!</h1>')


// `.example > div.trg.${targetDirection} > span.text
        const sourceExamples = $('.example > div.src > span.text').text().trim().split('\n')
...
/*
        const sourceExamples = $(
            `.example > div.src > span.text`
        )
            .text()
            .trim()
                 
*/
//	consol.mple > div.trg.${targetDirection} > span.textlog("souce   " + sourceExamples)

	    
	const targetExamples = $(
            `.example > div.trg > span.text`
        )
            .text()
            .trim()
            .split('\n')
