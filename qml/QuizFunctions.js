function initLangList() {
  ...
  idLangModel.append({
    "lang": "Japanese",
    "imgsource": "qrc:flags/japan-flag-3d-round-icon-128.png",
    "code": "ja"
  });
...
function lookUppInReverso() {
  ...
  var sUrl = "http://mss7000.com/glosquiz/revcontext.html?lang=" + sLang + "&word=" + encodeURI(oInText) + "&tlang=" + sTLang + "&slang=" + sFLang;
...
function getTextInputAndAdd() {
  ...
  for (i = 0; i < glosModel.count; i++) {
    if (sNewWordFrom.equalIgnoreCase(glosModel.get(i).question) && sNewWordTo.equalIgnoreCase(glosModel.get(i).answer)) {
...
function updateQuiz() {
  ...
  if (glosModel.get(idGlosList.currentIndex).question !== sQ || glosModel.get(idGlosList.currentIndex).answer !== sA_Org || glosModel.get(idGlosList.currentIndex).extra !== sE) {
...
function assigNextQuizWord() {
  ...
  if (idQuizModel.bDir === -1) {
    ...
    if (ii >= 0) {
      ...
      console.log("not found in working model " + nLastNumber + " " + glosModelWorking.count + " " + idQuizModel.get(i).question);
    }
...
function setAudioSpeedFromCarmodeSpeed() {
  if (idRectTakeQuiz.fCarModeSpeed1_10 >= 4) {
    MyDownloader.setAudioSpeed(idRectTakeQuiz.fCarModeSpeed1_10 / 5.5);
  }
}

function handleCarSlider(fV) {
  if (idRectTakeQuiz.fCarModeSpeed1_10 === fV)
    return;
  idRectTakeQuiz.fCarModeSpeed1_10 = fV;
  setAudioSpeedFromCarmodeSpeed();
}
...
function stopCarMode() {
  bCarMode = false;
  MyDownloader.setAudioSpeed(1.0);
  idCarTimer.stop();
  idMoveTimer.stop();
}
...
function exeCarMode() {
  /* Not needed siice the timer is stoped during movement
